...
// Import services
import { MongoWalletService, MongoChamaService } from './services';
...
const walletService = new MongoWalletService();
const chamaService = new MongoChamaService();
...
async function seedTestData() {
  ...
  // Use environment variable for the interval or default to 1 hour
  const hourlyInterval = parseInt(process.env.HOURLY_RULE_INTERVAL || '3600000', 10);
  walletBalanceRule.schedule = { interval: hourlyInterval };

  ruleEngine.addRule(walletBalanceRule);
  console.log(
    `Created rule(${new Date}): ${walletBalanceRule.name} (${walletBalanceRule.id})`
  );
}
...
app.post('/api/trigger-notification', (req: Request, res: Response) => {
  ...
  walletService
    ...
    .then((wallet) => {
      ...
      return chamaService
        .getChamaMembersByWalletId(walletId)
        .then((members) => {
          if (members.length === 0) {
    ...
    .catch((error) => {
      ...
      if (
        ...
        error.message !== 'Members not found'
      ) {
