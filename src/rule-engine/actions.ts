...
export class ConsoleNotificationService implements NotificationService {
    async sendNotification(recipient: Member, message: string): Promise<void> {
        console.log(`[NOTIFICATION] To: ${recipient.name} (${recipient.phone || recipient.npub || 'no contact'}): ${message}`);
    }

    async sendBulkNotifications(recipients: Member[], message: string): Promise<void> {
        console.log(`[BULK NOTIFICATION] Sending to ${recipients.length} members: ${message}`);
...
export class NotifyWalletBalanceAction implements Action {
    ...
    async execute(): Promise<void> {
        try {
            ...
            const members = await this.chamaService.getChamaMembersByWalletId(this.walletId);
            if (members.length === 0) {
